...
class ListeEnergie extends Component {
    constructor(props){
        super(props);
        this.state = {
            liste_energie :[]
    ...
    fetchData = () => {
       ...
       EnergieService.getAllEnergie(token,id).then((res)=> {
            this.setState({liste_energie:res.data});
        });
    }
    render() {
        return(
                    ...
                    <div className="pd-ltr-20 xs-pd-20-10">
                        ...
                        <div className="clearfix mb-20">
                            ...
                            <div className="pull-right">
                                <Link to={`/${sessionStorage.getItem("token")}/add_energie`} className="btn btn-primary btn-sm scroll-click" rel="content-y"  data-toggle="collapse" role="button">+ Ajouter une nouvelle energie</Link>
                            </div>
                        </div>
                        <table className="table">
                            ...
                            <tbody>
                                {
                                    this.state.liste_energie.map (
                                        modele=>
                                        <tr>
                                            <th scope="row">{modele.idEnergie}</th>
                                            <td>ENERGIE {modele.idEnergie}</td>
                                            <td>{modele.energie}</td>
                                            <td>
                                                <div>
