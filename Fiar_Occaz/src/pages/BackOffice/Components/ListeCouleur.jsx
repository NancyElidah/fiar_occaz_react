...
class ListeCouleur extends Component {
    constructor(props){
        super(props);
        this.state = {
            liste_couleur :[]
    ...
    fetchData = () => {
       ...
       CouleurService.getAllCouleur(token,id).then((res)=> {
            this.setState({liste_couleur:res.data});
        });
    }
    render() {
        return(
                    ...
                    <div className="pd-ltr-20 xs-pd-20-10">
                        ...
                        <div className="clearfix mb-20">
                            <div className="pull-left">
                                <h4 className="text-blue h4" style={{color:"blue"}}>Liste de tous les Couleurs </h4>
                                <p>Crud <code>.Model</code></p>
                            </div>
                            <div className="pull-right">
                                <Link to={`/${sessionStorage.getItem("token")}/add_couleur`} className="btn btn-primary btn-sm scroll-click" rel="content-y"  data-toggle="collapse" role="button">+ Ajouter une Couleur</Link>
                            </div>
                        </div>
                        <table className="table">
                            ...
                            <tbody>
                                {
                                    this.state.liste_couleur.map (
                                        modele=>
                                        <tr>
                                            <th scope="row">{modele.idcouleur}</th>
                                            <td>Couleur {modele.idcouleur}</td>
                                            <td>{modele.nom}</td>
                                            <td>
                                                <div>
                                                    <Link to={`update_couleur/${modele.idcouleur}`} className="btn btn-primary btn-sm scroll-click" rel="content-y"  data-toggle="collapse" role="button"><i className="fa fa-edit"></i></Link>
                                                </div>
                                            </td>
                                            <td>
                                                <div>
